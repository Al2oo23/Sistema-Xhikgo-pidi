...
if (isset($_POST['registrar']) && $_POST['registrar'] == 'registrar') {
    ...
    $abejas->setCi_otro($_POST['ci_otro']);
     ...
     foreach ($_POST['efectivos'] as $cedula) {
        ...
        $efectivo = $preparado->fetch(PDO::FETCH_ASSOC);
    ...
    for($i = 0; $i<count($_POST['recurso']);$i++){
        ...
        $preparado = $conexion->prepare($sql);
        $preparado->bindParam(':id', $id, PDO::PARAM_STR);
        $preparado->execute();
    
        $recurso = $preparado->fetch(PDO::FETCH_ASSOC)[0];

        if($recurso['cantidad'] >= $_POST['cantidad'][$i]){
            $caso2 = true;
    ...
    foreach ($_POST['unidad'] as $niv) {
        ...
        $sql = "SELECT * FROM vehiculo WHERE niv= :niv";
        $preparado = $conexion->prepare($sql);
        $preparado->bindParam(':niv', $niv, PDO::PARAM_STR);
        $preparado->execute();
    
        $unidad = $preparado->fetch(PDO::FETCH_ASSOC);
